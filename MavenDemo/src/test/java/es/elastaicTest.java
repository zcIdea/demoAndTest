...
public class elastaicTest {
	...
	public void before()
	{
		...
		Settings setting = ImmutableSettings.settingsBuilder()
			...
			.put("client.transport.sniff", true)//启动嗅探功能，开启client.transport.sniff功能，探测集群所有节点
			.put("client.transport.ping_timeout", "30s")//连接超时
			.build();
		...
		TransportAddress transportAddress = new InetSocketTransportAddress("169.254.182.148",9300);
...
//				.setQuery(QueryBuilders.matchQuery("name", "will").operator(Operator.AND)) //根据tom分词查询name,默认or(分词查询)
//				.setQuery(QueryBuilders.multiMatchQuery("will", "name", "age")) //指定查询的字段
//				.setQuery(QueryBuilders.queryString("name:will* AND age:[0 TO 19]")) //根据条件查询,支持通配符大于等于0小于等于19
//				.setQuery(QueryBuilders.termQuery("name", "will"))//查询时不分词
				.setSearchType(SearchType.QUERY_THEN_FETCH)
